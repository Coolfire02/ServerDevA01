...
public class PlayfabUserStatsManager : MonoBehaviour
{
    [SerializeField]
    ...
    {
        StartCoroutine(tryRefreshStats());
    ...
    {
        if (!(GetComponentInParent<Player>() != null && GetComponentInParent<Player>().GetPhotonView() != null
            && GetComponentInParent<Player>().GetPhotonView().IsMine)) return false;

        print("Refreshing stats");

        if (!PlayFabClientAPI.IsClientLoggedIn()) return false;
        if (isPreviousRefreshCompleted() || apiCallbacks < 0)
        {
            if (tx_coinsDisplay != null && tx_coinsDisplay.text.Length == 0) tx_coinsDisplay.text = "Fetching...";
            if (tx_levelDisplay != null && tx_levelDisplay.text.Length == 0) tx_levelDisplay.text = "Fetching...";
            ...
            PlayFabClientAPI.GetUserData(new GetUserDataRequest(),
                r =>
                {
                    ...
                    if (tx_levelDisplay != null)
                        tx_levelDisplay.text = "Level " + lvl + ", " + xp + "/" + PlayfabLeveling.getXPRequiredForLevel(lvl + 1) + " XP";
                },
                playFabUtils.GetComponent<PlayfabUserManagement>().OnError);
